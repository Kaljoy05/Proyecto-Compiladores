...
func (p *Parser) parseNotaStatement() *ast.NotaStatement {
	...
	if !isNotaValida(p.curToken.Type) {
		p.errors = append(p.errors, fmt.Sprintf("nota no válida: %s", p.curToken.Literal))
		return nil
	}
	...
	if p.curToken.Type == token.AGUDA || p.curToken.Type == token.GRAVE {
		stmt.Octava = p.curToken
		p.nextToken()
	}

	if !isDuracionValida(p.curToken.Type) {
		p.errors = append(p.errors, fmt.Sprintf("duración no válida: %s", p.curToken.Literal))
		return nil
	}
	stmt.Duracion = p.curToken

	if p.peekTokenIs(token.PUNTO_COMA) {
		p.nextToken()
	}
...
func (p *Parser) parseAcordeStatement() *ast.AcordeStatement {
	...
	if !isNotaValida(p.curToken.Type) {
		p.errors = append(p.errors, fmt.Sprintf("raíz de acorde no válida: %s", p.curToken.Literal))
		return nil
	}
	...
	if p.curToken.Type != token.MAYOR && p.curToken.Type != token.MENOR {
		p.errors = append(p.errors, fmt.Sprintf("se esperaba mayor/menor, se obtuvo: %s", p.curToken.Literal))
		return nil
	}
...
func (p *Parser) parseSilencioStatement() *ast.SilencioStatement {
	...
	if !isDuracionValida(p.curToken.Type) {
		p.errors = append(p.errors, fmt.Sprintf("duración de silencio no válida: %s", p.curToken.Literal))
		return nil
	}
	stmt.Duracion = p.curToken
